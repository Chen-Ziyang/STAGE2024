...
def center_rcrop(img, output_size):
    if len(img.shape) == 3:
        if isinstance(output_size, numbers.Number):
            output_size = (int(output_size), int(output_size))
        h, w = img.shape[0:2]
        ...
        i = int(round((d - td) / 2.))
        if i > 2:
            i += random.randrange(-i//2, i//2)
        j = int(round((h - th) / 2.))
        if j > 2:
            j += random.randrange(-j//2, j//2)
        k = int(round((w - tw) / 2.))
        if k > 2:
            k += random.randrange(-k//2, k//2)
...
def get_range_val(value, rnd_type="uniform"):
    # The code is copied from batchgenerators package.
