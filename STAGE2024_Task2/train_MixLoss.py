    ...
    for iter in tqdm(range(1, iters+1)):
        for batch, data in enumerate(train_dataloader):
            ...
            for i in range(len(logit_outputs)):
                if loss is None:
                    loss = criterion(labels, logit_outputs[i], 2)
                else:
                    loss += criterion(labels, logit_outputs[i], 2)
            ...
            r2 = R2_(labels.cpu(), logit_outputs[0].detach().cpu()).numpy()
...
def main(train_filelists, val_filelists, fold):
    ...
    train_dataset = STAGE_sub2_dataset(dataset_root=trainset_root,
                            ...
                            label_file=root + 'MICCAI2024-STAGE2/STAGE_training/training_GT/task2_GT_training.csv',
