...
class Resize3D:
    ...
    def __call__(self, img):
        depth_factor = self.output_shape[0] / img.shape[0]
        height_factor = self.output_shape[1] / img.shape[1]
        width_factor = self.output_shape[2] / img.shape[2]

        resized_img = zoom(np.squeeze(img, axis=-1), (depth_factor, height_factor, width_factor), order=3)
        return np.expand_dims(resized_img, axis=-1)
...
class CenterRandomCrop(object):
    ...
    def __call__(self, img):
        ...
        """
        return F.center_rcrop(img, self.size)
...
class GaussianBlurTransform(object):
    def __init__(self, p=0.2, p_per_channel=0.5, blur_sigma=(0.5, 1.), different_sigma_per_axis=True):
        self.p = p
    ...
    def __call__(self, img):
        if random.random() < self.p:
            return F.augment_gaussian_blur(img, p_per_channel=self.p_per_channel, sigma_range=self.blur_sigma,
...
class GaussianNoiseTransform(object):
    def __init__(self, p=0.2, p_per_channel=0.75):
        self.p = p
        self.p_per_channel = p_per_channel

    def __call__(self, img):
        if random.random() < self.p:
            return F.augment_gaussian_noise(img, p_per_channel=self.p_per_channel)
        return img
