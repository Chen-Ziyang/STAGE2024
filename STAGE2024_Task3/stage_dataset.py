...
class STAGE_sub3_dataset(data.Dataset):
    """
    getitem() output:
    ...
    def __init__(self,
        ...
        if self.mode == 'train':
            ...
            self.file_list = [[f, task3_label[int(f)], info[int(f)]] for f in os.listdir(dataset_root)]
        elif self.mode == "test":
            info = {row['ID']: [row[1], row[2], row[3], row[4]]
                        for _, row in pd.read_excel(info_file).iterrows()}
    ...
    def __getitem__(self, idx):
        real_index, task3_label, info = self.file_list[idx]
        if self.mode == 'train':
        ...
        if self.mode == "train":
            return oct_imgs, fundus_imgs, np.array(task3_label), np.array([gender[info[0]], info[1]/100., eye[info[2]], stage[info[3]]])
