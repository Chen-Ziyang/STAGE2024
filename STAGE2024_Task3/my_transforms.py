...
def collate_fn_w_transform(batch):
    oct_imgs, label1, label2, label3, info = zip(*batch)
    oct_imgs = np.stack(oct_imgs, 0)
    label1 = np.stack(label1, 0)
    label2 = np.stack(label2, 0)
    label3 = np.stack(label3, 0)
    ...
    return oct_imgs, label1, label2, label3, info
